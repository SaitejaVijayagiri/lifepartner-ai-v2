...
async function main() {
    const targets = await prisma.users.findMany({
        where: {
            OR: [
                { full_name: { contains: 'manibharathi', mode: 'insensitive' } },
                { full_name: { contains: 'mirza sameer', mode: 'insensitive' } },
                { full_name: { contains: 'nikhil kumar', mode: 'insensitive' } },
    ...
    for (const user of targets) {
        ...
        console.log(`✅ Removed photo for: ${user.full_name} (${user.email})`);
