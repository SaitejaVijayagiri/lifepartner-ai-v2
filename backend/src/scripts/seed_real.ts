...
async function seedReal() {
    ...
    try {
        ...
        for (const user of realProfiles) {
            // Upsert Logic: Delete if exists to ensuring fresh data
            ...
            const res = await client.query(`
                INSERT INTO public.users (email, password_hash, full_name, gender, age, location_name, avatar_url)
                VALUES ($1, $2, $3, $4, $5, $6, $7)
                RETURNING id
            `, [user.email, hashedPassword, user.name, user.gender, user.age, user.location.city, user.avatar]);
            ...
            const finalMetadata = {
                ...user.metadata,
                reels: user.reels // Storing reels in metadata
            };
            ...
            console.log(`✅ Created Real Profile: ${user.name}`);
        }

        await client.query('COMMIT');
        console.log("🎉 REAL PROFILES SEEDED!");
    } catch (e) {
