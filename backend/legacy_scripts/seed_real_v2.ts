...
async function seedRealV2() {
    console.log("🌸 SEEDING REALISTIC PROFILES V2 (WITH ROBUST INSERT & PHONE)...");
    ...
    try {
        ...
        for (const user of realProfiles) {
            ...
            // 1. Insert user with WORKING QUERY SUBSET (From auth.ts) + Phone (tested)
            // exclude avatar_url for now, add it via UPDATE
            const res = await client.query(`
                INSERT INTO public.users (
                    email, phone, password_hash, full_name, age, gender, location_name
                ) VALUES ($1, $2, $3, $4, $5, $6, $7)
                RETURNING id
            `, [user.email, user.phone, hashedPassword, user.name, user.age, user.gender, user.location.city]);
        ...
        console.log("🎉 REAL PROFILES V2 SEEDED SUCCESSFULLY!");
    } catch (e) {
