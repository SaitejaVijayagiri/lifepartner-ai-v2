...
export default function LiveVideoEventsHub({ onJoinLive }: LiveVideoEventsHubProps) {
    ...
    return (
        <div className="w-full space-y-6 animate-in fade-in duration-300">
            ...
            {/* Active Live Video Rooms Grid or Authentic Empty State */}
            {events.length > 0 ? (
                    ...
                    {events.map((event) => (
                        ...
                        >
                            ...
                            <div className="relative aspect-[16/9] w-full bg-gray-950 overflow-hidden">
                                <img
                                    src={event.host_avatar || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(event.host_name || 'Host')}`}
                                    alt={event.host_name}
                                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 filter brightness-90"
                                    onError={(e) => {
                                        (e.target as any).src = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(event.host_name || 'Host')}`;
                                    }}
                                />
                ...
                <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-br from-gray-900 via-purple-950/40 to-slate-900 border border-gray-800 text-center space-y-4 shadow-xl">
                    ...
                    <div className="space-y-1 max-w-md mx-auto">
                        <h3 className="text-lg sm:text-xl font-black text-white">No Active Live Streams Right Now</h3>
                        <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">
                            Be the first single to go LIVE! Host your video room or launch 1-on-1 speed dating.
                        </p>
                    </div>
                    <button
                        onClick={() => setShowHostModal(true)}
                        className="px-6 py-3 rounded-2xl bg-gradient-to-r from-rose-500 via-purple-600 to-indigo-600 hover:from-rose-600 hover:to-indigo-700 text-white font-extrabold text-xs shadow-lg shadow-rose-500/30 transition-all hover:scale-105 active:scale-95 inline-flex items-center gap-2"
                    >
                        <PlusCircle size={16} />
                        <span>Host Your Live Video Stream</span>
                    </button>
                </div>
            )}
